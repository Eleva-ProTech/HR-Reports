...
import { UserX, Clock3, CheckCircle2 } from 'lucide-react';
...
export default function TerminationReport() {
  ...
  const [terminationType, setTerminationType] = useState(pageFilters.termination_type || 'all');
  ...
  const buildParams = (overrides: Record<string, any> = {}) => ({
    ...
    termination_type: terminationType === 'all' ? undefined : terminationType,
  ...
  const applyFilters = () => {
    router.get(route('hr.reports.terminations'), buildParams(), { preserveState: true, preserveScroll: true });
  };
  ...
  const hasActiveFilters = () =>
    !!(
      ...
      dateTo ||
      (status && status !== 'all') ||
      (terminationType && terminationType !== 'all')
    );

  const activeFilterCount = () =>
    [branchId, departmentId, employeeId, dateFrom, dateTo, status !== 'all' ? status : '', terminationType !== 'all' ? terminationType : ''].filter(Boolean).length;
  ...
  const terminationTypeOptions = [
    ...
    ...(options?.termination_types || []).map((type: string) => ({
      value: type,
      label: type,
    })),
  ];
  ...
  return (
    <PageTemplate
      title={t('Termination Report')}
      description={t('Track involuntary employee exits and termination data')}
      url="/hr/reports/terminations"
      breadcrumbs={breadcrumbs}
    >
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-3">
          <SummaryCard
            title={t('Total Terminations')}
            value={stats?.total_terminations ?? 0}
            description={t('Terminations within the selected range')}
          ...
          />
          <SummaryCard
            title={t('Approved Terminations')}
            value={stats?.approved ?? 0}
            description={t('Approved terminations pending completion')}
        ...
        <Card>
          <CardContent className="space-y-4 pt-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex-1">
                <SearchAndFilterBar
                  ...
                  onPerPageChange={(value) =>
                    router.get(route('hr.reports.terminations'), buildParams({ per_page: value }), {
                      preserveState: true,
                      preserveScroll: true,
                    })
                  }
              ...
              <ExportButton
                exportRoute={route('hr.reports.terminations.export')}
                filters={{
                  search: searchTerm,
                  branch_id: branchId,
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t('Termination Type')}</TableHead>
                    <TableHead>{t('Termination Date')}</TableHead>
                ...
                <TableBody>
                  {records?.data?.length ? (
                    records.data.map((record: any) => {
                      ...
                      return (
                        <TableRow key={`termination-${record.id}`}>
                          <TableCell>
                          ...
                          <TableCell>{record.termination_type || '-'}</TableCell>
                          <TableCell>
                            {record.termination_date ? format(new Date(record.termination_date), 'PP') : '-'}
                          </TableCell>
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="py-6 text-center text-sm text-muted-foreground">
                        {t('No terminations found for the selected filters')}
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
